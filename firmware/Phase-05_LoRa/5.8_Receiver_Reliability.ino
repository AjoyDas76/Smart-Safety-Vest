/*
  ============================================================
  PHASE 5.8 - Packet Reliability & Error Handling (Receiver)
  ============================================================
  Adds on top of 5.7:
    1. Checksum validation - corrupted packets are dropped
       instead of trusted/displayed.
    2. Sends an ACK packet back to the transmitter whenever a
       valid SOS or FALL packet is received, so the worker unit
       knows the alert actually got through.
...
void setup() {
  ...
  Serial.println();
  Serial.println(F("=== Phase 5.8: Packet Reliability & Error Handling (Receiver) ==="));
  ...
  Serial.println(F("Type 'r' + Enter any time for a range-test summary."));
...
void loop() {
  ...
  if (packetSize != sizeof(DataPacket)) {
    // Not a data packet (could be a stray ACK-sized packet, or noise) - ignore quietly
  ...
  // ---- 5.8: ACK back for critical alerts ----
