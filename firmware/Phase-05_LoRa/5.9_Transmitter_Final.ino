/*
  ============================================================
  PHASE 5.9 - LoRa Communication Module Finalization (TRANSMITTER)
  ============================================================
  FIRMWARE_VERSION: LoRa Module v1.0 (Final)

  This is the finalized worker-unit LoRa transmitter for the
  Smart Safety Vest, closing out Phase 5. It combines everything
  built across 5.1-5.8 into one clean, production-ready sketch:

    - 5.1-5.3  Core LoRa init (SX1278 RA-02, SF9/125kHz/CR4-5)
    - 5.4      Worker status transmission (NORMAL / SOS)
    - 5.5      GPS data in every packet
    - 5.6      Fall detection (MPU-6050) -> automatic STATUS_FALL alert
    - 5.7      (range-testing was validated separately - see
               5.7_Transmitter_RangeTest.ino)
    - 5.8      Checksum + ACK/retry reliability for critical alerts

  No new logic is introduced here beyond 5.8 - this file exists
  so Phase 9 (System Integration) has one unambiguous "final"
  LoRa transmitter to build on top of, instead of picking between
  5.6/5.8 variants.
...
struct __attribute__((packed)) DataPacket {
  ...
  float    latitude;
  float    longitude;
  uint8_t  checksum;   // XOR over every byte above
};
...
/* ========================================================
   MPU-6050 Fall Detection (finalized logic from Phase 3.8)
   ======================================================== */
...
/* ========================================================
   Checksum + ACK/retry reliability layer (from 5.8)
   ======================================================== */
