...
class LiveChartFragment : Fragment() {
    ...
    // Wall-clock label for each tick, so the X axis can show real time
    // instead of a plain running counter. Index i corresponds to x = i + 1.
    ...
    private fun styleChart(chart: LineChart, dataSet: LineDataSet) {
        ...
        chart.xAxis.valueFormatter = object : ValueFormatter() {
            override fun getFormattedValue(value: Float): String {
                return timeLabels.getOrNull(value.toInt() - 1) ?: ""
            }
        }
    ...
    private fun onTelemetry(telemetry: VestTelemetryManager.Telemetry) {
        ...
        timeIndex += 1f
        timeLabels.add(timeFormat.format(Date()))
